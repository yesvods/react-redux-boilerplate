...
export function removeNode(tree, id){
  ...
  travesalTree(tree, tree['root'], node => {
    if(node.id === id){
      ...
      //找不到父元素，则不进行删除操作
      if(!pNode) return;
      let index = pNode.children.indexOf(id);
 ...
 * @param  {string} key     搜索属性值
 * @return {Objecet}        过滤后的树 
 */
export function filterTree(tree, keyWord, key='title'){
  let filterdTree = _.clone(tree, true);
  travesalTree(filterdTree, filterdTree['root'], node => {
    if(!_.contains(node[key], keyWord) && (!node.children || node.children.length===0)){
...
//nest tree to flag tree
export function n2f(node){
  if(!node) return {};
  
  if(node.children && node.children instanceof Array){
    let childNodes = node.children.reduce((memo, node) => {
      return Object.assign(memo, n2f(node))
    }, {});
    node.children = node.children.map(node => node.name);
